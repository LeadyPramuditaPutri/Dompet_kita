...
//        Log.d("ID",data.getSpent()));
    ...
    public void showReport(MenuItem menu){
        DbTabungan db = new DbTabungan(getApplicationContext());
        db.open();
        ArrayList<DbTabungan.Tabungan> data = db.selectCategory("Pemasukkan");
        float inc = 0;
        for(int i=0;i<data.size();i++){
//            Log.d("inc",String.valueOf(data.get(i).getSpent()));
            inc += Float.valueOf(data.get(i).getSpent());
        }
        ArrayList<DbTabungan.Tabungan> dataO = db.selectCategory("Pengeluaran");
        float out = 0;
...
//        Log.d("first",inc+"--"+out);
        ...
        i.putExtra("Pemasukkan",inc);
        i.putExtra("Pengeluaran",out);
        startActivity(i);
    }
